...
def _write_fake_codex(path: Path, output: str) -> None:
    path.write_text(
        f"""#!{sys.executable}
import base64
import json
...
pathlib.Path(os.environ["FAKE_CODEX_CAPTURE"]).write_text(json.dumps(capture))
print({output!r})
""",
        encoding="utf-8",
    )
...
async def test_run_codex_cli_uses_safe_flags_and_returns_html(
    ...
    _write_fake_codex(cli_path, json.dumps({"html": expected_html}))
...
def stop(*_):
    pathlib.Path(os.environ["FAKE_CODEX_TERMINATED"]).write_text("yes")
    raise SystemExit(0)
...
""",
    ...
    cli_path.chmod(0o755)
    monkeypatch.setenv("FAKE_CODEX_TERMINATED", str(terminated_path))
...
async def test_run_codex_cli_rejects_invalid_output(
    ...
    cli_path = tmp_path / "codex"
    _write_fake_codex(cli_path, output)
...
pathlib.Path(os.environ["FAKE_CODEX_STARTED"]).write_text("yes")
...
""",
    ...
    cli_path.chmod(0o755)
    monkeypatch.setenv("FAKE_CODEX_STARTED", str(started_path))
    monkeypatch.setenv("FAKE_CODEX_TERMINATED", str(terminated_path))
